\documentclass[bachelor,preview,makeindex,everb]{zzuThesis1.0}
...
\department{院系}
\submissiondate{完成时间}
\date{\makebox[10em][s]{\hspace{\fill}2017\hspace{\fill}年\hspace{\fill}5\hspace{\fill} 月
\hspace{\fill}8\hspace{\fill} 日}}
\index{封面}
\maketitle
...
公式的编号用圆括号括起，放在公式右边行末，在公式和编 号之间不加虚线。公式可按全文统编序号，也可按章单独立序号，如 $（49）$或$（4.11）$，采用哪一种序号应和稿中的图序、表序编法一致。 不应出现某章里的公式编序号，有的则不编序号。子公式可不编序号， 需要引用时可加编a、b、c……重复引用的公式不得另编新序号。
...
\begin{description}
  ...
  \item[\texttt{preview}] 生成适合于屏幕预览的交叉引用 (无该选项时生成的交叉
    引用适合于 PDF 文件)。
  \item[\texttt{makeindex}] 用 \verb|\index| 命令生成索引。
